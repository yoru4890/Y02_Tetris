...
void DX::TileI::InitTile(GameConstants::ShapeTile shapeTile)
{
...
void DX::TileI::CCWRotate(const std::array<GameConstants::ShapeTile, GameConstants::BOARD_SIZE>& m_board, int xDir, int yDir)
{
	if (!m_isCCWRotate)
	{
		...
		for (int i{}; i < 4; i++)
		{
			...
			else
			{
				...
				if (isCanRotate)
				{
					for (int j{}; j < m_tilePos.size(); j++)
					{
						m_tilePos[j].first = m_nextTilePos[j].first - xDir;
						m_tilePos[j].second = m_nextTilePos[j].second;
					}
					...
					m_position.x -= TILE_SIZE * xDir;
					m_position.y -= TILE_SIZE * yDir;
					m_state = (m_state + State::SIZE - 1) % State::SIZE;
					basePos.first -= xDir;
					basePos.second -= yDir;
					break;
...
void DX::TileI::CWRotate(const std::array<GameConstants::ShapeTile, GameConstants::BOARD_SIZE>& m_board, int xDir, int yDir)
{
	if (!m_isCWRotate)
	{
		for (int i{}; i < m_tilePos.size(); i++)
