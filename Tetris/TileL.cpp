...
void DX::TileL::InitTile(GameConstants::ShapeTile shapeTile)
{
	TileBase::InitTile(shapeTile);
}

void DX::TileL::Rotate(DirectX::Keyboard::State const& kb, const std::array<GameConstants::ShapeTile, GameConstants::BOARD_SIZE>& m_board)
{
	if (kb.Q)
	{
		switch (m_state)
...
void DX::TileL::CCWRotate(const std::array<GameConstants::ShapeTile, GameConstants::BOARD_SIZE>& m_board, int xDir, int yDir)
{
	if (!m_isCCWRotate)
	{
...
void DX::TileL::CWRotate(const const std::array<GameConstants::ShapeTile, GameConstants::BOARD_SIZE>& m_board, int xDir, int yDir)
{
	if (!m_isCWRotate)
	{
		for (int i{}; i < m_tilePos.size(); i++)
